        ...
        {stats.sectionStats.map(s => (
          <div key={s.section} style={{ background:'var(--bg2)', borderRadius:'var(--radius-lg)', border:`0.5px solid ${s.late>0?'rgba(244,91,91,0.3)':'var(--border)'}`, padding:'16px' }}>
            ...
            <div style={{ display:'flex', gap:16 }}>
              <div>
                <div style={{ fontSize:10, color:'var(--text3)', fontFamily:'var(--mono)', textTransform:'uppercase', marginBottom:2 }}>Completati</div>
                <div style={{ fontSize:20, fontWeight:600, color: s.done===s.total&&s.total>0?'var(--green)':'var(--text)' }}>{s.done}/{s.total}</div>
              </div>
              <div>
                <div style={{ fontSize:10, color:'var(--text3)', fontFamily:'var(--mono)', textTransform:'uppercase', marginBottom:2 }}>In ritardo</div>
                <div style={{ fontSize:20, fontWeight:600, color: s.late>0?'var(--red)':'var(--text3)' }}>{s.late}</div>
              </div>
            </div>
          </div>
        ))}
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--bg)' }}>
      ...
      <div style={{ background:'var(--bg2)', borderBottom:'0.5px solid var(--border)', padding:'0 1.5rem', display:'flex', alignItems:'center', height:52, gap:12, position:'sticky', top:0, zIndex:10 }}>
        ...
        <button onClick={signOut} style={{ fontSize:11, padding:'4px 10px', borderRadius:7, border:'0.5px solid var(--border2)', background:'transparent', color:'var(--text3)', cursor:'pointer', fontFamily:'var(--font)' }}>
          Esci
        </button>
      </div>
